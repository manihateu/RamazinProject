...
class products //объявляем шаблон структуры
{
	public:
		...
		void clearBD(vector <products>& mas) {
			cout << "\nБаза данных очищена\n\n";
			ofstream ofs;
		...
		void enter_new(vector <products>& mas, int &sch) //Функция ввода новой товара
		{
			system("cls");
		...
		void out(vector <products>& mas, int& sch) //Функция вывода записей
		{
			setlocale(LC_ALL, "Russian");
		...
		void find(vector <products>& mas, int& sch) //Функция поиска записей
		{
...
products Funcs; // создаём объект класса для вызова функций
...
int main()
{
	...
	while (1)
	{
		switch (Funcs.menu())
		{
		case 1:Funcs.enter_new(mas, sch); break;
		case 2:Funcs.find(mas, sch); break;
		case 3:Funcs.out(mas, sch); break;
		case 4:Funcs.clearBD(mas); break;
		case 5: return 0;
