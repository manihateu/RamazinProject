...
void clearBD() {
	cout << "\nБаза данных очищена\n\n";
	ofstream ofs;
	ofs.open("C:\\Users\\nik85\\OneDrive\\Рабочий стол\\RamazinProject\\SkladTovarov\\db.txt", ofstream::out | ofstream::trunc);
	ofs.close();
	mas.clear();
	system("pause");
}
...
void enter_new() //Функция ввода новой товара
{
	...
	if (1) //Вводим новую запись только, если счетчик полных записей меньше максимального количества записей
	{
		...
		fstream fs;
		fs.open("C:\\Users\\nik85\\OneDrive\\Рабочий стол\\RamazinProject\\SkladTovarov\\db.txt", ios::app);
			fs << mymas.product_name << "\n";
